  ...
  return (
      <div className={classes.paper} >
          ...
          <div className={classes.catPreview}>
              {/* call product cards */}
              { products.length ? (
                  products.map( product =>  <ProductCard parent={parent} key={ product.id } title={product.name} price={product.regularPrice} salePrice={product.salePrice} image={product.image.
                      sourceUrl} productId={product.productId} type={product.type} slug={product.slug}  /> )
                  ) : '' }
              {/* ========== */}
              {/* scroll fix */}
